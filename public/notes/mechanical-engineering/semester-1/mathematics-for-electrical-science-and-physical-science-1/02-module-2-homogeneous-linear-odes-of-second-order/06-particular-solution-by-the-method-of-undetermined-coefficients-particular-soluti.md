...
semester: 1
topicId: "68bd1edefeb4799d95e8354c"
status: "completed"
scrapedAt: "2026-05-20T17:39:21.738Z"
---
...
### General Strategy and Textbook References

The method of undetermined coefficients is a powerful shortcut for specific types of $f(x)$. Kreyszig's "Advanced Engineering Mathematics" (Chapter 2, Section 2.4) provides a thorough treatment of this method, including detailed derivations and more complex cases. Bird's "Higher Engineering Mathematics" also covers these topics comprehensively.

The core idea is to *match the form* of the forcing function. For $f(x)=ke^{\gamma x}$, the building block is $e^{\gamma x}$. If the system's characteristic equation has roots that are also $\gamma$, we need to "promote" our guess by multiplying by $x$ or $x^2$ to ensure linear independence from the complementary solution.

**Important Note:** This method is primarily for ODEs with *constant coefficients*. For variable coefficients or more complex forcing functions, other methods like Variation of Parameters are required.

### Sample Questions and Answers

**Question 1 (Conceptual):**
Why do we sometimes need to multiply our initial guess for $y_p$ by $x$ or $x^2$ when solving non-homogeneous ODEs?

**Answer:** We multiply by $x$ or $x^2$ to ensure that our proposed particular solution $y_p$ is linearly independent of the functions already present in the complementary solution $y_c$. If our guess for $y_p$ is already a solution to the homogeneous equation (i.e., it's part of $y_c$), substituting it into the non-homogeneous ODE will lead to $0 = f(x)$ (if $f(x) \neq 0$), which is a contradiction. Multiplying by $x$ or $x^2$ creates new, linearly independent forms that can satisfy the non-homogeneous equation. This directly addresses the issue of "resonance" or "forcing at a natural frequency."

**Question 2 (Exam-Oriented):**
Find a particular solution to the differential equation $y'' + 5y' + 6y = 2e^{-3x}$.

**Answer:**
The ODE is $y'' + 5y' + 6y = 2e^{-3x}$.
Here, $a=1$, $b=5$, $c=6$, $k=2$, and $\gamma=-3$.

1.  **Characteristic Equation:** $r^2 + 5r + 6 = 0$.
    Factoring gives $(r+2)(r+3) = 0$.
    The roots are $r_1 = -2$ and $r_2 = -3$.
    The complementary solution is $y_c(x) = c_1e^{-2x} + c_2e^{-3x}$.

2.  **Initial Guess for $y_p$:** Based on $f(x) = 2e^{-3x}$, our initial guess is $y_p(x) = Ae^{-3x}$.

3.  **Check for Collision:** Our guess $Ae^{-3x}$ is of the form $Ae^{\gamma x}$ where $\gamma = -3$.
    Is $\gamma = -3$ a root of the characteristic equation $r^2 + 5r + 6 = 0$?
    Yes, $r=-3$ is a root ($(-3)^2 + 5(-3) + 6 = 9 - 15 + 6 = 0$).
    Since $e^{-3x}$ is part of $y_c$, our initial guess collides.

4.  **Modify the Guess:** Since $\gamma = -3$ is a single root (not repeated, as the roots are -2 and -3), we modify our guess to $y_p(x) = Axe^{-3x}$.

5.  **Calculate Derivatives:**
    $y_p'(x) = A(1 \cdot e^{-3x} + x \cdot (-3)e^{-3x}) = A(1 - 3x)e^{-3x}$
    $y_p''(x) = A(-3e^{-3x}) + A(1 - 3x)(-3)e^{-3x} = A(-3 - 3(1 - 3x))e^{-3x} = A(-3 - 3 + 9x)e^{-3x} = A(-6 + 9x)e^{-3x}$

6.  **Substitute into ODE:**
    $A(-6 + 9x)e^{-3x} + 5A(1 - 3x)e^{-3x} + 6Axe^{-3x} = 2e^{-3x}$

7.  **Equate Coefficients:** Divide by $e^{-3x}$:
    $A(-6 + 9x) + 5A(1 - 3x) + 6Ax = 2$
    $-6A + 9Ax + 5A - 15Ax + 6Ax = 2$
    Collect terms:
    $(-6A + 5A) + (9A - 15A + 6A)x = 2$
    $-A + (0)x = 2$
    $-A = 2$
...
Therefore, the particular solution is $y_p(x) = -2xe^{-3x}$.

**Question 3 (Exam-Oriented):**
For the equation $y'' - 6y' + 9y = 5e^{3x}$, find the appropriate form for the particular solution $y_p$ and determine the coefficient $A$.

**Answer:**
The ODE is $y'' - 6y' + 9y = 5e^{3x}$.
Here, $a=1$, $b=-6$, $c=9$, $k=5$, and $\gamma=3$.

1.  **Characteristic Equation:** $r^2 - 6r + 9 = 0$.
    Factoring gives $(r-3)^2 = 0$.
    The characteristic equation has a repeated root $r=3$.
    The complementary solution is $y_c(x) = c_1e^{3x} + c_2xe^{3x}$.

2.  **Initial Guess for $y_p$:** Based on $f(x) = 5e^{3x}$, our initial guess is $y_p(x) = Ae^{3x}$.

3.  **Check for Collision:** Our guess $Ae^{3x}$ is of the form $Ae^{\gamma x}$ where $\gamma = 3$.
    Is $\gamma = 3$ a root of the characteristic equation $(r-3)^2 = 0$? Yes.
    Is it a single or repeated root? The root $r=3$ is repeated.
    This means $e^{3x}$ and $xe^{3x}$ are both in $y_c$. Our initial guess $Ae^{3x}$ collides. Our second guess $Axe^{3x}$ will also collide.

4.  **Modify the Guess (Double Collision):** Since $\gamma=3$ is a repeated root, we must use the form $y_p(x) = Ax^2e^{3x}$.

5.  **Determine the Coefficient A:** For a repeated root $\gamma$, where $a\gamma^2+b\gamma+c = 0$ and $2a\gamma+b=0$, the formula for $A$ is $A = \frac{k}{2a}$.
    Here, $a=1$, $k=5$.
    $A = \frac{5}{2(1)} = \frac{5}{2}$.

So, the appropriate form for the particular solution is $y_p(x) = Ax^2e^{3x}$ and the coefficient $A$ is $\frac{5}{2}$. The particular solution is $y_p(x) = \frac{5}{2}x^2e^{3x}$.