...
export default function LabRecordTracker({ branch, sem, triggerNotification }: LabRecordTrackerProps) {
  ...
  const completedExercises = activeLab?.exercises.filter(ex => ex.logic && ex.record && ex.viva && ex.signed).length || 0;
      ...
      <div className="bg-white/80 dark:bg-slate-950/60 backdrop-blur-3xl border border-slate-200/50 dark:border-white/[0.04] rounded-3xl p-5 md:p-6 shadow-xl space-y-5">
        ...
        <AnimatePresence>
          {showInfo && (
            ...
            >
              ...
              <p>For each experiment, complete all 4 milestones to mark it 100% done:</p>
              <ul className="list-disc pl-4 space-y-1 font-mono">
                ...
                <li><strong className="text-pink-600 dark:text-pink-400">Viva:</strong> Mini oral questions answered.</li>
        ...
        {labCourses.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none pb-1 border-b border-slate-100 dark:border-white/[0.03]">
              {labCourses.map(lab => (
                <button
                  key={lab.id}
                  onClick={() => {
                  ...
                  className={`px-3 py-1.5 rounded-2xl text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap active:scale-95 ${
                    activeLabTab === lab.id
                      ? "bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20"
                      : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 border border-transparent hover:bg-slate-100 dark:hover:bg-white/[0.02]"
                  }`}
                >
                  {lab.code}
                </button>
            ...
            {activeLab && (
              <div className="space-y-4">
                ...
                <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1 scrollbar-thin">
                  ...
                  <div className="hidden sm:grid grid-cols-12 gap-2 px-4 py-1.5 text-[9px] font-bold text-slate-450 uppercase tracking-widest items-center">
                    <div className="col-span-4">Experiment Title</div>
                    <div className="col-span-2 text-center">Logic</div>
                    <div className="col-span-2 text-center">Record</div>
                    <div className="col-span-2 text-center">Viva</div>
                    <div className="col-span-1 text-center">Signed</div>
                    <div className="col-span-1 text-center">Delete</div>
                  </div>

                  {activeLab.exercises.map((ex) => {
                    const isAllCompleted = ex.logic && ex.record && ex.viva && ex.signed;
                    
                    return (
                      <div 
                        key={ex.id}
                        className={`flex flex-col sm:grid sm:grid-cols-12 gap-3 items-stretch sm:items-center px-4 py-3 sm:py-2.5 rounded-2xl border transition-all duration-300 ${
                          isAllCompleted 
                      ...
                      >
                        ...
                        {/* Milestones row */}
                        <div className="flex sm:contents items-center justify-between gap-1.5 border-t border-slate-100 dark:border-slate-800/40 sm:border-0 pt-2.5 sm:pt-0">
                          
                          {/* Logic Milestone (Teal) */}
                          <div className="flex-1 sm:col-span-2 flex items-center justify-center gap-1.5">
                            <span className="sm:hidden text-[8px] font-bold text-slate-450 uppercase block">Logic:</span>
                            <button
                          ...
                          <div className="flex-1 sm:col-span-2 flex items-center justify-center gap-1.5">
                            <span className="sm:hidden text-[8px] font-bold text-slate-450 uppercase block">Record:</span>
                            <button
                          ...
                          {/* Viva Milestone (Pink) */}
                          <div className="flex-1 sm:col-span-2 flex items-center justify-center gap-1.5">
                            <span className="sm:hidden text-[8px] font-bold text-slate-450 uppercase block">Viva:</span>
                            <button
                              onClick={() => toggleMilestone(activeLab.id, ex.id, "viva")}
                              className={`w-7 h-7 sm:w-6 sm:h-6 flex items-center justify-center transition-all cursor-pointer rounded-full border active:scale-90 ${
                                ex.viva 
                                  ? "bg-pink-500 text-white border-transparent shadow-sm shadow-pink-500/20" 
                                  : "border-slate-300 dark:border-slate-700 bg-transparent text-slate-400 hover:text-slate-600 dark:text-slate-500"
                              }`}
                              aria-label={`Verify viva for ${ex.name}`}
                            >
                          ...
                          <div className="flex-1 sm:col-span-1 flex items-center justify-center gap-1.5">
                            <span className="sm:hidden text-[8px] font-bold text-slate-455 uppercase block">Signed:</span>
                            <button
                          ...
                          {/* Action (Delete) */}
                          <div className="flex justify-center items-center sm:col-span-1 pl-2 sm:pl-0 border-l border-slate-100 dark:border-slate-800/40 sm:border-none self-stretch sm:self-auto shrink-0">
                            <span className="sm:hidden text-[8px] font-bold text-slate-450 uppercase block mr-1.5">Delete:</span>
                            <button
