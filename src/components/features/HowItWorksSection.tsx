        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {steps.map((step, i) => {
            ...
            return (
              <motion.div
                key={i}
                className={`relative z-10 min-h-[220px] sm:min-h-[280px] md:h-[325px] p-5 sm:p-6 flex flex-col justify-between rounded-3xl border border-slate-200/50 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.015)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.25)] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-1.5 group ${step.hoverStyle}`}
                initial={{ opacity: 0, y: 20 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
