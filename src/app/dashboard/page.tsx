...
function DashboardContent() {
  ...
  return (
    <div
      className="h-screen overflow-hidden relative flex flex-col font-sans transition-colors duration-500 text-slate-900"
      style={{
    ...
    >
      ...
      <main className="relative flex-1 w-full max-w-7xl mx-auto px-4 md:px-6 pt-20 md:pt-32 flex overflow-y-auto lg:overflow-hidden z-10 scrollbar-none">
          ...
          {isTransitioning && (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <div className="w-24 h-4 bg-slate-200/50 animate-pulse rounded-md" />
              </div>

              {/* Title area skeleton */}
              <div className="space-y-3 mb-10">
                <div className="w-16 h-4 bg-slate-200/50 animate-pulse rounded-md" />
                <div className="w-2/3 h-10 bg-slate-200/50 animate-pulse rounded-xl" />
              </div>

              {/* Modules/Content cards skeleton */}
              <div className="space-y-5">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white/60 border border-slate-200/50 rounded-3xl p-5 space-y-4 shadow-sm">
                    <div className="flex justify-between items-center pb-3 border-b border-slate-100/50">
                      <div className="w-48 h-6 bg-slate-200/50 animate-pulse rounded-lg" />
                      <div className="w-12 h-4 bg-slate-200/50 animate-pulse rounded-md" />
                    </div>
                    <div className="space-y-3">
                      {[1, 2].map((j) => (
                        <div key={j} className="flex justify-between items-center py-1">
                          <div className="w-3/4 h-5 bg-slate-100/50 animate-pulse rounded-md" />
                          <div className="w-6 h-6 bg-slate-100/50 animate-pulse rounded-full" />
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            >
              <div className="lg:col-span-8 space-y-5 md:space-y-6 h-auto lg:h-full lg:overflow-y-auto pb-6 lg:pb-32 pr-1 md:pr-2">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-lg text-[9px] font-black tracking-widest uppercase bg-slate-950/[0.04] border border-slate-950/[0.06] text-slate-700 shadow-none">
                    ...
                    <h2 
                      ...
                      ref={headingRef}
                      tabIndex={-1}
                      className="text-2xl md:text-4xl font-black tracking-tight text-slate-900 leading-tight focus:outline-none"
                    >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 md:px-5 py-3.5 md:py-4 rounded-xl md:rounded-[20px] bg-white/65 hover:bg-white border border-slate-950/[0.06] hover:border-slate-950/[0.12] hover:shadow-[0_12px_40px_-12px_rgba(0,0,0,0.08)] cursor-pointer transition-all duration-300 text-slate-400 font-medium backdrop-blur-md text-left"
                ...
                >
                  ...
                  <div className="ml-auto hidden md:flex items-center gap-1 shrink-0">
                    <kbd className="px-2 py-0.5 rounded bg-slate-100 text-[10px] font-black text-slate-500 border border-slate-200">Ctrl</kbd>
                    <kbd className="px-2 py-0.5 rounded bg-slate-100 text-[10px] font-black text-slate-500 border border-slate-200">K</kbd>
                  </div>
                </button>

                {resumeTarget && (
                  <div className={`relative rounded-xl md:rounded-[20px] overflow-hidden bg-white/65 border transition-all duration-300 backdrop-blur-md p-5 md:p-8 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.08)] group hover:border-slate-950/[0.12] ${resumeTheme.border}`}>
                    ...
                    <div className={`absolute -bottom-6 -right-6 opacity-[0.07] pointer-events-none transform -rotate-12 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-700 ease-out ${resumeTheme.text}`}>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5 md:gap-6">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-3.5 h-3.5 text-slate-800 fill-current" />
                          <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest">UP NEXT IN {resumeTarget.subject.code}</span>
                        </div>
                        <h3 className="text-lg md:text-2xl font-black text-slate-900 tracking-tight leading-tight">{resumeTarget.topic.title}</h3>
                        <p className="text-[10px] md:text-xs font-semibold text-slate-400">{resumeTarget.module.title} • Resume learning session</p>
                      </div>
                      <MagneticButton
                ...
                <div className="space-y-5 md:space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg md:text-xl font-black text-slate-800 tracking-tight">Your Courses</h3>
                    <span className="text-[10px] md:text-xs font-bold text-slate-500">{subjects.length} active courses</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                    {!isLoaded
                      ? skeletonCards.slice(0, 4).map((item) => (
                          <div key={item} className="h-36 rounded-[20px] bg-white/40 animate-pulse border border-slate-950/[0.04] shadow-sm" />
                        ))
                          ...
                          const allTopicIds = subject.modules.flatMap((module) => module.topics.map((topic) => topic.id));
                           ...
                           return (
                            ...
                            >
                              ...
                              <div className="flex items-start justify-between gap-4 w-full">
                                  <div className="space-y-1 max-w-[calc(100%-3.2rem)]">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{subject.code}</span>
                                    </div>
                                    <h4 className="text-base font-black text-slate-900 leading-tight line-clamp-2">{subject.name}</h4>
                                  </div>

                                  <div className="relative shrink-0 w-11 h-11 flex items-center justify-center bg-white/80 rounded-full shadow-inner border border-slate-950/[0.04]">
                                    <svg className="w-11 h-11 transform -rotate-90 shrink-0" viewBox="0 0 40 40">
                                      <circle cx="20" cy="20" r={radius} className="stroke-slate-950/[0.03] fill-none" strokeWidth="2.5" />
                                      <circle
                                        cx="20"
                                        cy="20"
                                        r={radius}
                              ...
                              <div className="mt-6 pt-4 border-t border-slate-950/[0.06] flex items-center justify-between w-full text-[11px] font-semibold text-slate-400/50">
                                <span>{subject.modules.length} Modules • {allTopicIds.length} Topics</span>
                                <div className={`flex items-center gap-1 text-slate-400 group-hover:${scheme.text} group-hover:translate-x-0.5 transition-all`}>
                                  <span>Open</span>
          ...
          {!isTransitioning && view === "topic" && selectedTopic && (
            ...
            >
              ...
              >
                <div className="mb-6 md:mb-8 border-b border-slate-100 pb-6 md:pb-8 pt-6 md:pt-8 flex items-start justify-between gap-4 max-w-3xl mx-auto px-4 md:px-0">
                  <h1 
                    ...
                    ref={headingRef}
                    tabIndex={-1}
                    className="text-2xl md:text-5xl font-black text-slate-900 tracking-tight leading-[1.1] focus:outline-none"
                  >
                    {selectedTopic.title}
                  </h1>
                  <button
                    ...
                    className={`h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 transition-colors ${pinnedTopicIds.includes(selectedTopic.id) ? "bg-amber-100 text-amber-600" : "bg-slate-50 text-slate-400 hover:bg-amber-50 hover:text-amber-500"}`}
                ...
                {selectedTopic.pyqs && selectedTopic.pyqs.length > 0 && (
                  <div className="mt-10 md:mt-12 max-w-3xl mx-auto bg-gradient-to-br from-indigo-50 to-white border border-indigo-200/70 rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-[0_8px_30px_rgba(99,102,241,0.08)] relative overflow-hidden mx-4 md:mx-0">
                    <div className="flex items-center gap-2 mb-3 md:mb-4 relative z-10">
                      ...
                      <span className="text-[10px] md:text-xs font-black text-indigo-600 uppercase tracking-widest">Model Question (2024 Scheme)</span>
                      <span className="ml-auto text-[10px] md:text-xs font-bold text-indigo-600 bg-indigo-100 px-3 py-1 rounded-full">Model Paper</span>
                    </div>
                    <p className="text-sm md:text-base font-bold text-slate-800 leading-snug mb-4 relative z-10">
                      &quot;{selectedTopic.pyqs[0].question}&quot;
                    </p>
                    <div className="flex justify-end relative z-10">
                      <span className="text-[10px] md:text-xs font-black text-indigo-500 px-3 py-1 rounded-lg bg-indigo-50 border border-indigo-100">{selectedTopic.pyqs[0].marks} Marks</span>
                    </div>
                  </div>
                )}

                {/* Recommended Watch Section */}
                <div className="mt-8 max-w-3xl mx-auto bg-white border border-slate-950/[0.04] rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm relative overflow-hidden mb-8 md:mb-12 mx-4 md:mx-0">
                  <div className="flex items-start justify-between mb-5 md:mb-6">
                    <div>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">RECOMMENDED WATCH</h4>
                      <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">Curated for you</h2>
                    </div>
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          ...
                          <h3 className="text-xs md:text-sm font-black text-slate-800 leading-tight mb-1 group-hover:text-blue-600 transition-colors line-clamp-2">{card.title}</h3>
              ...
              <div className="fixed bottom-6 left-0 right-0 z-[49] flex justify-center px-4 pointer-events-none">
                <div className="pointer-events-auto bg-white/70 backdrop-blur-xl border border-white/60 p-2 rounded-2xl md:rounded-3xl flex items-center gap-2 md:gap-4 w-full max-w-3xl shadow-[0_20px_60px_rgba(0,0,0,0.06),0_4px_16px_rgba(0,0,0,0.03),inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <button 
                    onClick={() => prevTopic && goTopic(prevTopic, selectedSubject ?? undefined)} 
                    disabled={!prevTopic} 
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-3 md:px-4 py-2.5 md:py-3 rounded-xl md:rounded-2xl text-xs md:text-sm font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 text-slate-600"
          ...
          >
            ...
            >
              ...
              <div className="max-h-[420px] overflow-y-auto p-3">
                {searchTerm.trim() === "" && (
                  <p className="px-3 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">All Topics</p>
                )}
                {searchTerm.trim() !== "" && filteredTopics.length > 0 && (
                  <p className="px-3 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">{filteredTopics.length} result{filteredTopics.length !== 1 ? 's' : ''} found</p>
                )}
                {filteredTopics.slice(0, 10).map((item) => {
                  ...
                  return (
                    <button
                      ...
                      key={item.topic.id}
                      onClick={() => goTopic(item.topic, item.subject)}
                      className="w-full flex items-center gap-3 rounded-2xl px-3 py-3 text-left hover:bg-blue-50 transition-colors group"
                    >
                      <div className={`h-10 w-10 rounded-xl grid place-items-center shrink-0 transition-colors ${done ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-50 text-blue-500 group-hover:bg-blue-100'}`}>
                        ...
                        <span className="block text-sm font-black text-slate-900 truncate">
                          {matchIdx >= 0 ? (
                            <>
                              ...
                              <mark className="bg-blue-200/70 text-blue-900 rounded px-0.5">{title.slice(matchIdx, matchIdx + query.length)}</mark>
                              {title.slice(matchIdx + query.length)}
                            </>
                          ) : title}
                        </span>
                        <span className="block text-xs font-bold text-slate-400">{item.subject.code} · {item.module.title}</span>
                      </span>
                      <ArrowRight className="ml-auto w-4 h-4 text-slate-300 group-hover:text-blue-500 group-hover:translate-x-0.5 transition-all shrink-0" />
                    </button>
                  );
                })}
                {filteredTopics.length === 0 && (
                  <div className="py-10 text-center">
                    ...
                    <p className="text-sm font-black text-slate-400">No matching topic found.</p>
                    <p className="text-xs text-slate-300 mt-1">Try searching by subject code or module name</p>
                  </div>
                )}
              </div>
            </motion.div>
