...
export default async function AdminDashboard({ searchParams }: PageProps) {
  ...
  // Server Action to save or upload dynamic note files
  async function saveNoteFile(formData: FormData) {
    "use server";
  ...
  // Server Action to save raw JSON config editor text
  async function saveRawConfig(jsonText: string) {
    "use server";
    try {
  ...
  // Server Action to save dynamic timetable overrides
  async function saveTimetableOverride(branch: string, sem: number, exams: any[]) {
    "use server";
    try {
  ...
  // Server Action to save dynamic FAQ overrides
  async function saveFaqOverride(faqs: any[]) {
    "use server";
    try {
      const currentConfig = getConfig();
  ...
  // Server Action to save dynamic quick links overrides
  async function saveQuickLinksOverride(links: any[]) {
    "use server";
    try {
      const currentConfig = getConfig();
  ...
  // Server Action to update site configuration dynamically
